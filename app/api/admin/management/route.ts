...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error.code === '23505' || error.cause?.code === '23505') {
      return NextResponse.json({ error: "Data sudah terdaftar (duplikat nama/kode/email)." }, { status: 400 });
    }
    return NextResponse.json({ error: "Terjadi kesalahan internal pada server database: " + error.message }, { status: 500 });
  }
}
