...
export default function DashboardClient() {
  ...
  const handleTestSpam = async () => {
    ...
    try {
      ...
      setModalConfig({
        isOpen: true,
        title: "Pengujian Triage AI Berhasil",
        description: "Sukses menyuntikkan 50 aduan spam dan berhasil dikliring secara otomatis oleh AI!",
        type: "success"
      });
    } catch (err: any) {
      setModalConfig({
        ...
        description: err.message || "Terjadi kesalahan saat pengujian.",
        type: "error"
      });
    } finally {
      setTesting(false);
    ...
    <main className={styles.dashboard}>
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.navigation} aria-label="Menu dashboard">
          ...
          <button type="button" onClick={() => window.location.assign("/dashboard/manajemen")} className={active === "Manajemen" ? styles.navActive : ""}><UserCircleIcon size={18} /><span>Manajemen OPD</span></button>
